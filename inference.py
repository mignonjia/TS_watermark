...
def main(args):
    ...
    if args.load_fp16:
        ...
        d_type = torch.float
    model = OPTForCausalLM.from_pretrained("facebook/opt-1.3b", torch_dtype=d_type, device_map="auto").to(device)
    ...
    for inc in range(len(ckpt_paths)): 
        ...
        # Construct the data filtering/sampling scheme partials
            ...
            for step, batch in enumerate(loader):
                ...
                with torch.no_grad():
                    with autocast():
                        ...
                        ppl_list['wm'].append(torch.mean(output).item())
            ...
            print("="*term_width)
            print("ppl wm",  math.exp(statistics.mean(ppl_list['wm'])))
            cur_result['wm']['ppl'] = math.exp(statistics.mean(ppl_list['wm']))
        ...
        json_result['ckpt_'+str(ckpt_ids[inc])+'00'] = ckpt_result
    
        name_fp16 = '' 
        if args.load_fp16:
            name_fp16 = '_half'
        with open(f"eval/opt/wm_{args.split}/wm_{args.split}_{args.gamma}_{args.delta}{name_fp16}.json", "w") as outfile:
