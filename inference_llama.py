...
from watermark_processor_llama import WatermarkLogitsProcessor, WatermarkDetector
...
delta_init_list = ['1.0', '1.0', '1.25', '1.5', '1.75', '2.0']
gamma_init_list = ['0.1'] + ['0.25'] * 5 
ckpt_paths = [f"ckpt/llama/init_{gamma_init_list[i]}_{delta_init_list[i]}.pth" for i in range(len(delta_init_list))]
...
def detect(input_text, args, embed_matrix=None, device=None, tokenizer_opt=None, tokenizer_llama=None,  use_ckpt=True, gamma=None, delta=None):
    """Instantiate the WatermarkDetection object and call detect on
        the input text returning the scores and outcome of the test"""
    watermark_detector = WatermarkDetector(vocab=list(tokenizer_llama.get_vocab().values()),
                                        ckpt_path=args.ckpt_path,
                                        seeding_scheme=args.seeding_scheme,
                                        device=device,
                                        tokenizer_opt=tokenizer_opt,
                                        tokenizer_llama=tokenizer_llama,
...
def main(args):
    ...
    tokenizer_llama = LlamaTokenizer.from_pretrained("meta-llama/Llama-2-7b-hf", padding_side="left")
    model_llama = LlamaForCausalLM.from_pretrained("meta-llama/Llama-2-7b-hf", torch_dtype=d_type).cuda()
    model_llama.config.pad_token_id = tokenizer_llama.pad_token_id = 0  # unk
    model_llama.config.bos_token_id = 1
    model_llama.config.eos_token_id = 2
    ...
    if args.load_fp16:
        embed_loaded = torch.load(f'eval/no_wm_gen_embedding/embed_{args.split}_half_llama.pt')
    else: 
        embed_loaded = torch.load(f'eval/no_wm_gen_embedding/embed_{args.split}_llama.pt')
        
    for inc in range(len(ckpt_paths)): 
        ckpt_result = {}
        args.ckpt_path = ckpt_paths[inc]
        args.delta = delta_init_list[inc]
        args.gamma = gamma_init_list[inc]
        ckpt_result['init_gamma'] = args.gamma
        ckpt_result['init_delta'] = args.delta
        ...
        for setting in [2]:
            ...
            def data_collator(batch):
                ...
                input_ids_batch = []
                for item in batch:
                    input_ids = tokenizer_llama(item['truncated_input'], return_tensors="pt")["input_ids"]
                ...
                input_ids_padded_reversed = pad_sequence(input_ids_reversed, batch_first=True, padding_value=tokenizer_llama.pad_token_id)
            ...
            z_score_pos, z_score_att_pos = {'wm':0, 'wm_bs':0}, {'wm':0, 'wm_bs':0}
            term_width = 80
            ...
            for step, batch in enumerate(loader):
                ...
                input_ids = batch['input_ids'].to(model_llama.device)
                attention_masks = (input_ids != tokenizer_llama.pad_token_id).long()
                
                # full_ids = prompt['untruncated_inputs'].to(device) 
                prefix_len = input_ids.shape[1]

                watermark_processor = WatermarkLogitsProcessor(vocab=list(tokenizer_llama.get_vocab().values()),
                                                        ckpt_path=args.ckpt_path,
                                                        seeding_scheme=args.seeding_scheme,
                                                        embed_matrix = embed_matrix,
                                                        tokenizer_llama = tokenizer_llama,
                ...
                with torch.no_grad():
                    with autocast():
                        ...
                        torch.manual_seed(hash_key)
                        output_w_wm = model_llama.generate(input_ids, 
                                        **sample,
                                        logits_processor=LogitsProcessorList([watermark_processor]))

                        gamma_list['wm'].append(torch.mean(watermark_processor.gamma_list).item())
                        delta_list['wm'].append(torch.mean(watermark_processor.delta_list).item())
                    
                decoded_output_w_wm = tokenizer_llama.batch_decode(output_w_wm[:,prefix_len:], skip_special_tokens=True)
                ...
                for idx in range(args.batch_size):
                    with torch.no_grad():
                        with autocast():
                            wm_detection_result, _ = detect(decoded_output_w_wm[idx], 
                                                                    args, 
                                                                    embed_matrix=embed_matrix,
                                                                    device=device, 
                                                                    tokenizer_opt=tokenizer_opt,
                                                                    tokenizer_llama=tokenizer_llama,)
                            human_detection_result, _ = detect(human[idx], 
                                                                    args, 
                                                                    embed_matrix=embed_matrix,
                                                                    device=device, 
                                                                    tokenizer_opt=tokenizer_opt,
                            ...
                            z_score_list['wm'].append(float(wm_detection_result[3][1]))
                            z_score_list['human'].append(float(human_detection_result[3][1]))
                                

                    simcse_list['wm'].append(cos(embed_wm[idx], embed_no_wm[idx]).item())
                    # print(simcse_list['wm'][-1])
            
            cur_result['wm'] = {}
            cur_result['human'] = {}

            print("="*term_width)
            z_score_list['human'] = sorted(z_score_list['human'])[::-1]
            thres_0 = z_score_list['human'][0] + 0.001
            thres_1 = z_score_list['human'][5] + 0.001
            thres_4 = z_score_list['human'][20] + 0.001
            thres_5 = z_score_list['human'][25] + 0.001
            thres_10 = z_score_list['human'][50] + 0.001
            cur_result['human']['z'] = {
                    'avg': statistics.mean(z_score_list['human']),
                    'stdev': statistics.stdev(z_score_list['human']),
                    'total': len(z_score_list['human']),
                    'max': max(z_score_list['human']),
                    'thres_0': thres_0,
                    'thres_1': thres_1,
                    'thres_4': thres_4,
                    'thres_5': thres_5,
                    'thres_10': thres_10,
                    'full': z_score_list['human']
                }
            cur_result['wm']['z'] = {
                    'avg': statistics.mean(z_score_list['wm']),
                    'stdev': statistics.stdev(z_score_list['wm']),
                    'total': len(z_score_list['wm']),
                    'thres_0': sum([1 for val in z_score_list['wm'] if val > thres_0]),
                    'thres_1': sum([1 for val in z_score_list['wm'] if val > thres_1]),
                    'thres_4': sum([1 for val in z_score_list['wm'] if val > thres_4]),
                    'thres_5': sum([1 for val in z_score_list['wm'] if val > thres_5]),
                    'thres_10': sum([1 for val in z_score_list['wm'] if val > thres_10]),
                    'full': sorted(z_score_list['wm'])[::-1]
                }
            
            print("="*term_width)
            print("simcse wm", statistics.mean(simcse_list['wm']), statistics.stdev(simcse_list['wm']))
            cur_result['wm']['simcse'] = statistics.mean(simcse_list['wm'])

            print("="*term_width)
            print("gamma list wm", statistics.mean(gamma_list['wm']))
            cur_result['wm']['gamma'] = statistics.mean(gamma_list['wm'])
    
            print("="*term_width)
            print("delta list wm", statistics.mean(delta_list['wm']))
            cur_result['wm']['delta'] = statistics.mean(delta_list['wm'])
            print("="*term_width)

            ckpt_result['setting_'+str(setting)] = cur_result
    
        with open(f"eval/llama/wm_{args.split}/{args.gamma}_{args.delta}.json", "w") as outfile:
            outfile.write(json.dumps(ckpt_result, indent=4))
