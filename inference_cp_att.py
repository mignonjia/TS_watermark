...
from watermark_processor_cp_att import WatermarkLogitsProcessor, WatermarkDetector
...
from inference_bs_cp_att import insert_watermark
...
ckpt_paths = [f"ckpt/opt/init_{gamma_init_list[i]}_{delta_init_list[i]}.pth" for i in range(len(delta_init_list))]
ckpt_ids = [6,8,9,9,8,12]
...
def detect(input_text, args, embed_matrix=None, device=None, tokenizer=None, use_ckpt=True, gamma=None, delta=None, num_cp_split=1):
...
def main(args):
    ...
    for ckpt in ckpt_paths: # ckpt_list 
        ckpt_result = {}
        args.ckpt_path = ckpt
        ...
        for setting in [2]:
            ...
            for step, batch in enumerate(loader):
                ...
                decoded_prompt = tokenizer.batch_decode(output_w_wm[:,:prefix_len], skip_special_tokens=True)
                ...
                for idx in range(args.batch_size):
                    try:
                        with autocast():
                            text_cp_att = insert_watermark(decoded_output_w_wm[idx], decoded_prompt[idx], num_cp_split=args.num_cp_split)
                            wm_detection_result, _ = detect(text_cp_att, 
                                                                    args, 
                                                                    embed_matrix=embed_matrix,
                                                                    device=device, 
                                                                    tokenizer=tokenizer,
                                                                    num_cp_split=args.num_cp_split)
                            human_detection_result, _ = detect(decoded_prompt[idx], 
                                                                    args, 
                                                                    embed_matrix=embed_matrix,
                                                                    device=device, 
                                                                    tokenizer=tokenizer,
                    ...
                    except Exception: # if detect of one sentence is unsuccessful, this case is pretty rare
                        print("=" * 80)
                        print("sample idx", step * args.batch_size + idx)
                        print("=" * 80)
                        continue    
                    ...
                    if args.verbose:
                        print("#"*term_width)
                        print("Output no wm:")
                        print(decoded_output_no_wm[idx])
                        print("#"*term_width)
                        print("Output with watermark:")
                        print(decoded_output_w_wm[idx])
                        print("#"*term_width)
        ...
        json_result[ckpt] = ckpt_result
    
    with open(f"eval/cp_att_{args.num_cp_split}/wm/wm_{args.split}.json", "w") as outfile:
