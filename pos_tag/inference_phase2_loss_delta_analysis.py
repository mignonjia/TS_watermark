...
def main(args):
    ...
    print("CKPT_init:", args.ckpt_path)
    ...
    choices = []
    if args.short:
        choices.append(['short', True, 1.0])
        choices.append(['short', False, 1.0])
    if args.mid:
        choices.append(['mid', True, 1.0])
        choices.append(['mid', False, 1.0])
        ...
        for step, batch in enumerate(loader):
            ...
            with torch.no_grad():
                with autocast():
                    ...
                    g_list.extend(watermark_processor.gamma_list.tolist())
                    d_list.extend(watermark_processor.delta_list.tolist())
                    ent_list.extend(watermark_processor.ent.tolist())
                    prob_list.extend(watermark_processor.top_prob.tolist())
                    
                    output_wm_ids = output_w_wm[0][(prefix_len-1):-1]
                    print("gamma mean stdev", statistics.mean(g_list), statistics.stdev(g_list))
                    print("delta mean stdev", statistics.mean(d_list), statistics.stdev(d_list))
                    for i in range(200):
                        output_tks.append(tokenizer.decode(output_wm_ids[i]))
                    output_ids.extend(output_wm_ids.tolist())

    with open(f"delta_analysis/delta_gamma.json", "w") as outfile:
        outfile.write(json.dumps([g_list, d_list, ent_list, prob_list, output_tks, output_ids], indent=4))
