...
class WatermarkBase:
    def __init__(
        ...
        self.vocab = vocab
        self.opt_vocab_size = 50272
        self.llama_vocab_size = 32000
        ...
        if use_ckpt:
            ...
            self.delta_network.delta[-1].bias += delta_inc
        ...
        self.ent = torch.empty(0, dtype=torch.float).cuda()
        self.top_prob = torch.empty(0, dtype=torch.float).cuda()
    ...
    def _get_greenlist_ids(self, input_ids: torch.LongTensor) -> list[int]:
        ...
        greenlist_size = int(self.llama_vocab_size * gamma)
        vocab_permutation = torch.randperm(self.llama_vocab_size, device=input_ids.device, generator=self.rng)
...
class WatermarkLogitsProcessor(WatermarkBase, LogitsProcessor):
    ...
    def __call__(self, input_ids: torch.LongTensor, scores: torch.FloatTensor) -> torch.FloatTensor:
        ...
        # ent = -torch.where(raw_probs > 0, raw_probs * raw_probs.log(), raw_probs.new([0.0])).sum(dim=-1, keepdim=True)
        # top_prob, _ = torch.topk(raw_probs, 1, dim=1)
        # self.ent = torch.cat([self.ent, ent[0]])
        # self.top_prob = torch.cat([self.top_prob, top_prob[0]])
    ...
    def detect(
        ...
        **kwargs,
        ...
        if tokenized_text is None:
            assert self.tokenizer_llama is not None, (
