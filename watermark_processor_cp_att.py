...
class WatermarkDetector(WatermarkBase):
    def __init__(
        ...
        **kwargs,
        ...
        self.num_cp_split = num_cp_split
    ...
    def _compute_z_score(self, green_token_mask):
        # count refers to number of green tokens, T is total number of tokens
        T = len(green_token_mask)
        if self.num_cp_split == 1:
            win_size = 200
        elif self.num_cp_split == 3:
            win_size = 60
        max_z = -100
        for idx in range(0, T - win_size, 10):
            start, end = idx, idx + win_size
            var = torch.sum(self.gamma_list[start:end] * (1 - self.gamma_list[start:end]))
            mean = torch.sum(self.gamma_list[start:end])
            z = (sum(green_token_mask[start:end]) - mean)/torch.sqrt(var) 
            max_z = max(z.cpu(), max_z) 
        return max_z
    ...
    def _score_sequence(
        ...
        if return_z_score:
            score_dict.update(dict(z_score=self._compute_z_score(green_token_mask)))
        if return_p_value:
