...
init_delta_list = [1.0, 1.3, 1.5, 1.7, 2.0, 2.3]
...
def main(args):
    ...
    for inc in range(len(init_gamma_list)):
        ...
        for setting in [2]:
            ...
            z_score_list = {'wm':[], 'wm_bs':[], 'human': [], 'human_bs': [], 'para':[], 'para_bs':[]}
            ...
            with open(f"dipper/bs/{args.gamma}_{args.delta}.jsonl_pp", "r") as f:
            ...
            for step in range(25):
                ...
                for idx, dd in enumerate(data[step*args.batch_size:(step+1)*args.batch_size]):
                    prefix = dd["prefix"]
                    human = dd["gold_completion"]
                    text_no_wm = dd["no_wm_completion"]
                    text_w_wm_bs = dd["gen_completion"]
                    ...
                    output_w_wm_bs = tokenizer(text_w_wm_bs, return_tensors="pt", add_special_tokens=False)["input_ids"][0].to(device)
                    output_para = tokenizer(text_para, return_tensors="pt", add_special_tokens=False)["input_ids"][0].to(device)
                    if output_w_wm_bs.shape[0] > 500:
                        output_w_wm_bs = output_w_wm_bs[:500]
                    if output_para.shape[0] > 500:
                        output_para = output_para[:500]
                    output_w_wm_bs = torch.cat([input_ids[-5:], output_w_wm_bs], dim=0).unsqueeze(0)
                    ...
                    with torch.no_grad():
                        with autocast():
                            ...
                            z_score_list['para_bs'].append(float(wm_para_detection_result[3][1]))
                    ...
                    simcse_list['para_bs'].append(cos(embed_para[0], embed_no_wm_list[step*args.batch_size + idx]).item())
            ...
            cur_result['para_bs']['z'] = {
                    'avg': statistics.mean(z_score_list['para_bs']),
                    'stdev': statistics.stdev(z_score_list['para_bs']),
                    'total': len(z_score_list['para_bs']),
                    'thres_0': sum([1 for val in z_score_list['para_bs'] if val > thres_0]),
                    'thres_1': sum([1 for val in z_score_list['para_bs'] if val > thres_1]),
                    'thres_4': sum([1 for val in z_score_list['para_bs'] if val > thres_4]),
                    'thres_5': sum([1 for val in z_score_list['para_bs'] if val > thres_5]),
                    'thres_10': sum([1 for val in z_score_list['para_bs'] if val > thres_10]),
                    'full': sorted(z_score_list['para_bs'])[::-1]
                }
            print("="*term_width)
            cur_result['para_bs']['simcse'] = statistics.mean(simcse_list['para_bs'])

            json_result['setting_'+str(setting)] = cur_result
    
        with open(f"dipper/bs_eval/{args.gamma}_{args.delta}.json", "w") as outfile:
