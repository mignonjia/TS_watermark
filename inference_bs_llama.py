...
init_delta_list = [0.8, 0.9, 1.0, 1.3, 1.5, 1.7, 2.0, 2.3, 2.5]
...
def main(args):
    ...
    embed_out = {'short': {'multi': None, 'greedy': None}, 'mid': {'multi': None, 'greedy': None}}
    ...
    for inc in range(len(init_gamma_list)):
        ...
        for setting in [2]:
            ...
            for step, batch in enumerate(loader):
                ...
                watermark_processor_bs = WatermarkLogitsProcessor(vocab=list(tokenizer_llama.get_vocab().values()),
                                                        ckpt_path=args.ckpt_path,
                                                        seeding_scheme=args.seeding_scheme,
                                                        embed_matrix = embed_matrix,
                                                        use_ckpt = False,
                ...
                with torch.no_grad():
                    with autocast():
                        torch.manual_seed(hash_key)
                        output_no_wm = model_llama.generate(input_ids, 
                                        **sample)
                        
                        torch.manual_seed(hash_key) 
                        output_w_wm_bs = model_llama.generate(input_ids, 
                                        **sample,
                                        logits_processor=LogitsProcessorList([watermark_processor_bs]))

                decoded_output_no_wm = tokenizer_llama.batch_decode(output_no_wm[:,prefix_len:], skip_special_tokens=True)
                decoded_output_w_wm_bs = tokenizer_llama.batch_decode(output_w_wm_bs[:,prefix_len:], skip_special_tokens=True)
 
                output_no_wm_opt = tokenizer_opt(decoded_output_no_wm, padding=True, truncation=True, return_tensors="pt").to(device)
                output_wm_bs_opt = tokenizer_opt(decoded_output_w_wm_bs, padding=True, truncation=True, return_tensors="pt").to(device)
                
                with torch.no_grad():
                    with autocast():
                        embed_no_wm = model_simcse(**output_no_wm_opt, output_hidden_states=True, return_dict=True).pooler_output
                        embed_wm_bs = model_simcse(**output_wm_bs_opt, output_hidden_states=True, return_dict=True).pooler_output
               
                cos = torch.nn.CosineSimilarity(dim=0, eps=1e-6)
                embed_no_wm_list = torch.cat([embed_no_wm_list, embed_no_wm], dim=0)
                    ...
                    with torch.no_grad():
                        with autocast():
                            wm_detection_result_bs, _ = detect(decoded_output_w_wm_bs[idx], 
                                                                    ...
                                                                    gamma=args.gamma,
                                                                    delta=args.delta)
                            human_detection_result_bs, _ = detect(decoded_output_no_wm[idx], 
                                                                    args, 
                                                                    embed_matrix=embed_matrix,
                                                                    device=device, 
                                                                    tokenizer_opt=tokenizer_opt,
                            ...
                            z_score_list['human_bs'].append(float(human_detection_result_bs[3][1]))
                    
                    simcse_entry = cos(embed_wm_bs[idx], embed_no_wm[idx]).item()
                    simcse_list['wm_bs'].append(simcse_entry)
            ...
            print("z_wm_bs", cur_result['wm_bs']['z']['avg'])
            print("human_bs", cur_result['human_bs']['z']['avg'])
